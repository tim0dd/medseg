...
from medseg.models.segmentors.hardnet.non_local import NonLocal2d
...
class LawinHead5(nn.Module):
    def __init__(self, embed_dim=768, use_scale=True, reduction=2, in_channels=[140, 420, 720, 1200], num_classes=1,
                 norm_cfg=dict(type='BN', requires_grad=True), **kwargs):
        super(LawinHead5, self).__init__()
        self.norm_cfg = norm_cfg
        self.lawin_8 = LawinAttn(in_channels=512, reduction=reduction, use_scale=use_scale, norm_cfg=self.norm_cfg,
                                 mode='embedded_gaussian', head=64, patch_size=8)
        self.lawin_4 = LawinAttn(in_channels=512, reduction=reduction, use_scale=use_scale, norm_cfg=self.norm_cfg,
                                 mode='embedded_gaussian', head=16, patch_size=8)
        self.lawin_2 = LawinAttn(in_channels=512, reduction=reduction, use_scale=use_scale, norm_cfg=self.norm_cfg,
                                 mode='embedded_gaussian', head=4, patch_size=8)
