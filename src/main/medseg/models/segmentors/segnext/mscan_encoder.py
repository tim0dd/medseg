...
from medseg.models.common.weight_init import constant_init, normal_init, trunc_normal_init
...
class MSCAN(nn.Module):
    ...
    def init_weights(self):
        if self.pretrained_filename is None:
        ...
        elif isinstance(self.pretrained_filename, str):
            checkpoint_path = PathBuilder.pretrained_dir_builder().add(self.pretrained_filename).build()
            ...
            for k in list(loaded_state_dict.keys()):
                if k not in self.state_dict().keys():
                    ...
                    loaded_state_dict[k] = loaded_state_dict[k].to(self.state_dict()[k].dtype)
            self.load_state_dict(loaded_state_dict, strict=True)
        else:
            raise TypeError('pretrained_filename must be a str or None')
